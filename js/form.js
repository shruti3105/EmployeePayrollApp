...
function createAndUpdateStorage() {
  ...
  if (employeeList) {
    let employee = employeeList.find(
      (emp) => emp._id == employeePayrollObj._id
    );
...
const setEmployeePayrollObject = () => {
  ...
  employeePayrollObj._picture = document.querySelector(
    "input[name = profile]:checked"
  ).value;
  employeePayrollObj._gender = document.querySelector(
  ...
  employeePayrollObj._startDate = new Date(
    parseInt(year),
    parseInt(month) - 1,
    parseInt(day)
  );
